...
public class CameraMovement11 : MonoBehaviour
{
    #region References
    ...
    private void HandleHorizontalMovement(Vector3 playerPosVP)
    {
        ...
        if (!isChangingAreas)
        {
            ...
            if (playerRightXVP > leftInnerLimitX && playerLeftXVP < rightInnerLimitX)
            {
                ...
                {
                    //Ensures camera wont jump back if player retreats
                    if ((currentFocus == Side.Left && newPos.x > transform.position.x) ||
                        (currentFocus == Side.Right && newPos.x < transform.position.x))
